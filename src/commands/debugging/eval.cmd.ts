...
export const command = new Command("eval", "Eval code.", [], [], [{ name: "code", type: String, required: true }] as const, permissions.developer)
	.setExec(async(client, message, args, lang) => {
		...
		const type: string = evaled === undefined || evaled === null ? String(evaled) : evaled.constructor?.name ?? Object.getPrototypeOf(evaled) === null ? `Object: null prototype` : "AAAAAAAAAAAAA";
