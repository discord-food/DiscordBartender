...
export const command = new Command("prepare", "Prepare your current claimed order.", [], ["pp"], [] as const, permissions.staff)
	.setExec(async(client, message, args, lang) => {
		...
		const checkPrepared = async() => order.type.recipe.every(x => order.prepared.includes(x.id)) && (order.status = Status.PENDING_DELIVERY, await order.save(), message.channel.send("The order is fully prepared! It is now pending delivery."));
