...
`, true);
		...
		const order = client.models.Orders.create({ description, type, user: message.author.id, metadata: { channel: message.channel.id, gagi: message.author.tag } });
		await order.save();
		account.balance -= type.price;
		await account.save();
		await message.channel.send(lang.commands.order.success.format(order.type.name, order.id));
		await client.mainChannels.get("brewery")?.send(`🎫<@&748732674885288036> A new order with the ID \`${order.id}\` for \`${order.descriptor}\` has been placed by **${message.author.tag}** from **${message.guild?.name}**!`);
	});
