...
export const command = new Command("guildinfo", "Gets info about the guild.", ["serverinfo"], ["gi", "si"], [] as const, permissions.everyone)
	.setExec(async(client, message, args, lang) => {
		...
		const { _ } = client;
		const nl = { length: 0 };
		const groupedChannels = Object.assign({ txt: nl, category: nl, news: nl, voice: nl, store: nl, unknown: nl },
		...
		await (async() => {
			...
			embed
				.addField("Channels", `**${channels.size}** channels in total.
${channellength("text")}
${channellength("category")}
${channellength("news")}
${channellength("voice")}
${channellength("store")}
${channellength("unknown")}`, true)
				.addField("Creation Date", `Created at **${creationDate.calendar()}**\n(${creationDate.fromNow()})`, true)
