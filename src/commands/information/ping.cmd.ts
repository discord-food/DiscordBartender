...
export const command = new Command("ping", "Gets the bot ping.", [], [], "[verbose:bool]", permissions.everyone)
	.setExec(async (client, message, args, lang) => {
		...
		if (verbose) {
			...
			const speed = [0, 2e+7, 7e+7, 2e+8, 5e+9, 1e+10].findIndex(x => diff < x)
			const speedstr = lang.commands.ping.speeds[5 - Math.max(0, speed)]
			client.log(diff)
			await msg.edit(client.utils.format(lang.commands.ping.pong, pms(diff / 1000000, { formatSubMilliseconds: true }), speedstr))
		}
	});
