...
export const command = new Command("help", "Gets the command list.", ["?"], ["hp"], [] as const, permissions.everyone)
	.setExec(async(client, message, args, lang) => {
		...
		const fields: (Omit<EmbedField, "inline"> & { inline?: boolean })[] = Object.entries(client._.groupBy(commands, "category"))
			.flatMap(([category, categoryCommands]) => [{ name: client.EMPTY, value: `__**== ${category.toUpperCase()} ==**__`, inline: false }, ...categoryCommands.map(x => ({ name: `${x.name} ${x.syntaxString}`, value: `**Aliases**: ${x.aliases.join(", ") || "[None]"}\n**Shortcuts**: ${x.shortcuts.join(", ") || "[None]"}\n**Permission**: \`${x.permissionLevel.name.toUpperCase()}\`\n${x.description}` }))]);
		const chunked = client._.chunk(fields, 25);
		for (const [index, embedFields] of chunked.entries()) {
			const embed = new client.Embed();
			if (!index) {
				embed.setTitle(lang.commands.help.title.format(index + 1, chunked.length))
					.setDescription(lang.commands.help.description);
			}
			for (const field of embedFields) embed.addField(field.name, field.value, field.inline);
