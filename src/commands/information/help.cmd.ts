...
export const command = new Command("help", "Gets the command list.", ["?"], ["hp"], "[command:str]", permissions.everyone)
	.setExec(async (client, message, args, lang) => {
		...
		const fields: EmbedField[] = Object.entries(client._.groupBy(commands.array(), "category"))
			.flatMap(([category, categoryCommands]) => [{ name: `== ${category.toUpperCase()} ==`, value: client.EMPTY }, ...categoryCommands.map(x => ({ name: `${x.name} ${x.syntax}`, value: `**Aliases**: ${x.aliases.join(", ") || "[None]"}\n**Shortcuts**: ${x.shortcuts.join(", ") || "[None]"}\n**Permission**: ${x.permissionLevel.name.toUpperCase()}\n${x.description}` }))]);
		const chunked = client._.chunk(fields, 25);
		for (const [index, embedFields] of chunked.entries()) {
			const embed = new client.Embed()
				.setTitle(client.utils.format(lang.commands.help.title, index + 1, chunked.length))
