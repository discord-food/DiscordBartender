import { Message, MessageEmbed, TextChannel, Channel, Collection, User, GuildMember } from "discord.js";
...
export const getText = async (message: Message, display: string = "Respond with text.", time: number = 40000, filter: CallableFunction = (m: Message) => m.author!.id === message.author!.id): Promise<string | undefined> => {
...
export const getOptionalText = async (message: Message, display: string = "Respond with text.", time: number = 40000, filter: CallableFunction = (m: Message) => m.author!.id === message.author!.id): Promise<string | false | undefined> => {
...
export const getIndex = async<T>(message: Message, list: any[], internal: T[] = list.map((x, i) => x === null ? list[i] : x), display: string = "item"): Promise<getIndexReturnVal<T> | false> => {
...
export const getUser = async (message: Message, toParse: string, { autoself = false, filter = (member: GuildMember) => true }): Promise<User | null> => {
	...
	const userlist = client.mainGuild!.members.concat(message.guild!.members)
		...
		.sort((x, y) => compareUsers(toParse, (y[0] as GuildMember).user) - compareUsers(toParse, (x[0] as GuildMember).user)) as [GuildMember, number][];
