...
export const handler = async(message: Message) => {
	...
	const forbidden = ["!", ";", "/"];
	if (!prefix && !forbidden.some(x => message.content.startsWith(x)) && (message.content.split(/\s+/).length > 1)) return client.models.Messages.insert({ id: message.id, content: Util.cleanContent(message.content, message), author: message.author.id });
	if (!prefix) return;
